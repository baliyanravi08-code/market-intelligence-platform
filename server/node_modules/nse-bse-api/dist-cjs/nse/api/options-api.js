...
class OptionsApi {
     ...
     */
    async getOptionChainContractInfo(symbol) {
        return await this.httpClient.request(`${index_js_1.BASE_URL}/option-chain-contract-info`, {
    ...
    async getExpiryDatesV3(symbol) {
        ...
        if (contractInfo?.expiryDates && Array.isArray(contractInfo.expiryDates)) {
            return contractInfo.expiryDates.sort((a, b) => {
    ...
    async getOptionChainV3(params) {
        ...
        return await this.httpClient.request(`${index_js_1.BASE_URL}/option-chain-v3`, query);
    }
    /**
     ...
     */
    async getFilteredOptionChainV3(symbol, expiry, strikeRange = 10) {
        ...
        const atmStrike = uniqueStrikes.reduce((prev, curr) => Math.abs(curr - underlying) < Math.abs(prev - underlying) ? curr : prev);
    ...
    static calculateMaxPain(optionChain, expiryDate) {
        ...
        for (const x of optionChain.records.data) {
            ...
            const itemExpiry = x.expiryDates || x.expiryDate;
            if (itemExpiry !== expiryDateStr)
                continue;
            const expiryStrike = x.strikePrice;
            let pain = 0;
            for (const y of optionChain.records.data) {
                const yExpiry = y.expiryDates || y.expiryDate;
                if (yExpiry !== expiryDateStr)
                    continue;
    ...
    async compileOptionChain(symbol, expiryDate) {
        const expiry = (0, date_formatter_js_1.formatDateExpiry)(expiryDate);
     ...
     */
    async compileOptionChainV3(symbol, expiry) {
     ...
     */
    static calculateMaxPainV3(optionChain, expiry) {
        const painMap = {};
        for (const x of optionChain.records.data) {
            if (x.expiryDates !== expiry)
                continue;
            const expiryStrike = x.strikePrice;
            let pain = 0;
            for (const y of optionChain.records.data) {
                if (y.expiryDates !== expiry)
                    continue;
                const diff = expiryStrike - y.strikePrice;
                if (diff > 0 && y.CE)
                    pain += -diff * y.CE.openInterest;
