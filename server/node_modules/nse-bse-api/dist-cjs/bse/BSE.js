...
const SymbolParser_js_1 = require("./utils/SymbolParser.js");
...
class BSE {
    ...
    symbolParser;
    constructor(config = {}) {
        this.dir = (0, helpers_js_1.getPath)(config.downloadFolder || './downloads', true);
        this.symbolParser = new SymbolParser_js_1.SymbolParser();
    ...
    async download(url, folder, params, filename) {
        ...
        await throttle_js_1.Throttle.check();
        try {
            ...
            if (response.status === 404) {
                throw new index_js_1.BSEError('Report is unavailable or not yet updated.');
            }
            ...
            if (error.code === 'ECONNABORTED') {
                throw new index_js_1.TimeoutError('Request timed out');
            }
            throw error;
        }
    }
    async request(url, params, timeout = 10000) {
        try {
            ...
            if (error.response) {
                throw new index_js_1.ConnectionError(`${error.response.status}: ${error.response.statusText}`);
            }
            throw error;
        }
    }
    async lookup(scrip) {
        ...
        await throttle_js_1.Throttle.check('lookup');
    ...
    async bhavcopyReport(date, folder) {
        const targetFolder = folder ? (0, helpers_js_1.getPath)(folder, true) : this.dir;
        const dateStr = (0, helpers_js_1.formatDate)(date);
        ...
        if (!fs.existsSync(filePath)) {
            throw new index_js_1.BSEError(`Failed to download file: ${path.basename(filePath)}`);
        }
        return filePath;
    }
    /**
    ...
    async deliveryReport(date, folder) {
        ...
        // Extract zip file
        filePath = await (0, helpers_js_1.unzipFile)(filePath, targetFolder);
    ...
    async announcements(options = {}) {
        ...
        if (fromDate > toDate) {
            throw new index_js_1.BSEError("'fromDate' cannot be greater than 'toDate'");
        }
        if (subcategory !== '-1' && category === '-1') {
            throw new index_js_1.BSEError(`Specify a 'category' for subcategory: ${subcategory}`);
        }
        ...
        const params = {
            ...
            strPrevDate: (0, helpers_js_1.formatDate)(fromDate),
            strToDate: (0, helpers_js_1.formatDate)(toDate),
            strSearch: 'P',
            strscrip: scripcode,
            strType: segmentType,
        };
    ...
    async actions(options = {}) {
        ...
        if (fromDate && toDate) {
            ...
            params.Fdate = (0, helpers_js_1.formatDate)(fromDate);
            params.TDate = (0, helpers_js_1.formatDate)(toDate);
        }
        if (purposeCode) {
    ...
    async resultCalendar(options = {}) {
        ...
        if (fromDate && toDate) {
            ...
            params.fromdate = (0, helpers_js_1.formatDate)(fromDate);
            params.todate = (0, helpers_js_1.formatDate)(toDate);
        }
        if (scripcode) {
            params.scripcode = scripcode;
        }
    ...
    async gainers(options = {}) {
        ...
        if (by === 'group') {
            ...
            if (!BSE.validGroups.includes(groupName.toUpperCase())) {
                throw new index_js_1.BSEError(`${groupName}: Not a valid BSE stock group`);
            }
    ...
    async losers(options = {}) {
        ...
        if (by === 'group') {
            const groupName = name || 'A';
            if (!BSE.validGroups.includes(groupName.toUpperCase())) {
                throw new index_js_1.BSEError(`${groupName}: Not a valid BSE group`);
            }
            params.IndxGrpval = groupName.toUpperCase();
        }
        else if (by === 'index') {
    ...
    async listSecurities(options = {}) {
        ...
        if (group && !BSE.validGroups.includes(group.toUpperCase())) {
            throw new index_js_1.BSEError(`${group} not a valid BSE stock group`);
        }
        await throttle_js_1.Throttle.check();
        const response = await this.request(url, params);
        return response.data;
    ...
    async getScripName(scripcode) {
        ...
        throw new index_js_1.BSEError(`Could not find scrip name for ${scripcode}`);
    }
    /**
    ...
    async getScripCode(scripname) {
        ...
        throw new index_js_1.BSEError(`Could not find scrip code for ${scripname}`);
    }
    /**
    ...
    async fetchAllIndicesDataByDate(date) {
        await throttle_js_1.Throttle.check();
        const dateStr = (0, helpers_js_1.formatDateSlash)(date);
    ...
    async fetchHistoricalIndexData(options) {
        ...
        if (toDate < fromDate) {
            throw new index_js_1.BSEError('`toDate` must be greater than `fromDate`');
        }
        await throttle_js_1.Throttle.check();
        const targetFolder = folder ? (0, helpers_js_1.getPath)(folder, true) : this.dir;
        const fname = `${index}_${(0, helpers_js_1.formatDate)(fromDate)}_${(0, helpers_js_1.formatDate)(toDate)}.csv`;
        const filePath = await this.download(`${BSE.apiUrl}/ProduceCSVForDate/w`, targetFolder, {
            strIndex: index,
            dtFromDate: (0, helpers_js_1.formatDateSlash)(fromDate),
            dtToDate: (0, helpers_js_1.formatDateSlash)(toDate),
            period: period,
        }, fname);
