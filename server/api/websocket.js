 ...
 *
 * FIX-PRICE-1 — Stale/previous-day prices (THIS FIX):
 *   All subscriptions were using "ltpc" mode which sends ONE snapshot per
 *   connect and then goes silent. This caused prices to freeze at the
 *   previous day's close. Fix: changed index and stock subscriptions to
 *   "full" mode which streams continuous real-time ticks on every price
 *   change. Options stay on "full_d30" (unchanged).
 *
 * EXISTING FIXES (kept):
...
function subscribeStocksForBacktest(symbols) {
  ...
  if (streamer) {
    // FIX-PRICE-1: was "ltpc" — sends only one snapshot, then goes silent.
    // "full" streams continuous ticks on every price change.
    try { streamer.subscribe(newKeys, "full"); } catch (e) {
...
function subscribeWithInstrumentKeys(instrKeys, symbolMap) {
  ...
  if (streamer) {
    // FIX-PRICE-1: was "ltpc" — feeds marketScanner, needs live ticks.
    // "full" streams continuous ticks on every price change.
    try { streamer.subscribe(newKeys, "full"); } catch (e) {
...
function resubscribeAll() {
  ...
  // FIX-PRICE-1: was "ltpc" — sends only one snapshot then goes silent.
  // "full" streams continuous real-time ticks on every price change.
  try { streamer.subscribe(INDEX_INSTRUMENTS, "full"); } catch (e) {
    console.log("⚠️  Upstox index subscribe error:", e.message);
  }

  if (optionInstruments.size > 0) {
    // Options stay on "full_d30" — unchanged, already correct.
  ...
  if (stockInstruments.size > 0) {
    // FIX-PRICE-1: was "ltpc" — backtest stocks also need live ticks.
    try { streamer.subscribe(Array.from(stockInstruments), "full"); } catch (e) {
      console.log("⚠️  Upstox stock re-subscribe error:", e.message);
    }
  }

  if (stockInstrumentKeys.size > 0) {
    // FIX-PRICE-1: was "ltpc" — feeds marketScanner, must be live.
    try { streamer.subscribe(Array.from(stockInstrumentKeys), "full"); } catch (e) {
