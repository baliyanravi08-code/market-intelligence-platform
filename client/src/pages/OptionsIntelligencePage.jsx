...
// After download, rename: OptionsIntelligencePage.js → OptionsIntelligencePage.jsx
...
function OIPanel({ oi, nearATMSignals, tailRiskSignals, spot, activeSymbol }) {
  ...
  return (
    <PanelWrap>
      ...
      {oiSummary&&<div style={{ fontSize:7, fontFamily:"IBM Plex Mono,monospace", color:oiColor, marginBottom:6, padding:"3px 6px", background:`${oiColor}10`, borderRadius:3, border:`1px solid ${oiColor}22` }}>◈ {oiSummary}</div>}
...
// ════════════════ Gann Panel ══════════════════════════════════════════════════
// FIX: handles 3 states: null (waiting), marketClosed (server said closed), real data

function GannPanel({ gann }) {
  // IST clock for closed message
  ...
  // ── State 1: null = still waiting for server ───────────────────────────────
  if (!gann) return (
    <PanelWrap>
  ...
  // ── State 2: server replied — market closed or no data ─────────────────────
...
function MarketStructurePanel({ structure, gannData, gannBadgeMap, activeSymbol, spot, callWall, putWall, gammaFlip, maxPain, pcr, skew25 }) {
  ...
  let rangePct=null;
  if(callWall&&putWall&&spot){ rangePct=Math.round(((spot-putWall)/(callWall-putWall))*100); rangePct=Math.max(0,Math.min(100,rangePct)); }
  ...
  return (
    <PanelWrap>
      ...
      {gannData&&!gannData.marketClosed&&<div style={{ marginBottom:6 }}><GannBadge symbol={activeSymbol} gannMap={gannBadgeMap} compact={false} /></div>}
      {spot&&(
      ...
      {structure.ivEnvironment&&<div style={{ fontSize:8, fontFamily:"IBM Plex Mono,monospace", color:"#a0b8cc", marginTop:6 }}>IV env: <span style={{ color:"#ffd54f" }}>{structure.ivEnvironment.replace(/_/g," ")}</span></div>}
      {structure.straddlePrice!=null&&<div style={{ display:"flex", justifyContent:"space-between", fontSize:9, fontFamily:"IBM Plex Mono,monospace", padding:"5px 0", borderTop:"1px solid #1a3040", marginTop:6 }}><span style={{ color:"#c8d8e8" }}>ATM Straddle</span><span style={{ color:"#e8f2ff", fontWeight:700 }}>₹{fmt2(structure.straddlePrice)}</span></div>}
      {gannData?.keyLevels?.masterAngle!=null&&!gannData.marketClosed&&<div style={{ display:"flex", justifyContent:"space-between", fontSize:9, fontFamily:"IBM Plex Mono,monospace", padding:"5px 0", borderTop:"1px solid #1a3040" }}><span style={{ color:"#c8d8e8" }}>Gann 1×1</span><span style={{ color:"#ffd54f", fontWeight:700 }}>₹{Math.round(gannData.keyLevels.masterAngle).toLocaleString("en-IN")}</span></div>}
    </PanelWrap>
  );
}
...
export default function OptionsIntelligencePage({ socket }) {
  ...
  const requestGann=useCallback((sym,ltp)=>{ if(!socket||!sym) return; socket.emit("get-gann-analysis",{ symbol:toGannSym(sym), ltp }); },[socket]);
  ...
  useEffect(()=>{
    ...
    const onIntel=(payload)=>{
      ...
      setActiveSymbol(prev=>{ if(!prev) setTimeout(()=>requestGann(sym,(payload?.data||payload)?.ltp||null),100); return prev||sym; });
    ...
    const onGannAlert=(alert)=>{
      ...
      (alert.alerts||[]).forEach(a=>{ setLiveAlerts(prev=>[{...a,symbol:alert.symbol,ts:Date.now()},...prev].slice(0,30)); });
      setGannMap(prev=>{ const ex=prev[alert.symbol.toUpperCase()]; if(!ex) return prev; return {...prev,[alert.symbol.toUpperCase()]:{...ex,alerts:[...(alert.alerts||[]),...(ex.alerts||[]).filter(a=>a.priority!=="HIGH")]}}; });
    };
    const onMarketAlert=(alert)=>{ if(!alert) return; setLiveAlerts(prev=>[{...alert,ts:Date.now()},...prev].slice(0,30)); };
    ...
    return ()=>{ socket.off("options-intelligence",onIntel); socket.off("gann-analysis",onGann); socket.off("gann-alert",onGannAlert); socket.off("market-alert",onMarketAlert); };
  },[socket,requestGann]);

  const handleSymbolChange=(sym)=>{ setActiveSymbol(sym); const payload=data[sym],d=payload?.data||payload||{}; requestGann(sym,d?.ltp||d?.spot||null); };

  const current=data[activeSymbol]||null, d=current?.data||current||null;
  const score=d?.score??null, bias=d?.bias??"NEUTRAL";
  const band=score!=null?ScoreBand(score):null;
  const vol=d?.volatility||{}, greeks=d?.atmGreeks||{}, gex=d?.gex||{};
  const dealerExp=d?.dealerExposures||{}, oi=d?.oi||{}, structure=d?.structure||{}, strategy=d?.strategy||[];

  const gannData=activeSymbol?(gannMap[toGannSym(activeSymbol)]||gannMap[activeSymbol]||gannMap[activeSymbol?.toUpperCase()]||null):null;
  ...
  if(hasDedicatedFields){ nearATMSignals=oiNear; tailRiskSignals=oiTail; }
  else{ const allUnusual=oi.unusualOI||[]; nearATMSignals=filterByProximity(allUnusual,spot,activeSymbol,nearATMPct); const nearSet=new Set(nearATMSignals.map(u=>`${u.strike}-${u.type}`)); tailRiskSignals=allUnusual.filter(u=>!nearSet.has(`${u.strike}-${u.type}`)); }
            ...
            <div style={{ flexShrink:0, background:band?.bg||"#060f1c", border:`1px solid ${band?.color||"#1c3a58"}44`, borderRadius:6, padding:"8px 14px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div style={{ flexShrink:0, minWidth:120 }}>
                  ...
                  <div style={{ display:"flex", gap:3, flexWrap:"wrap" }}>{strategy.slice(0,4).map((s,i)=><StrategyTag key={i} signal={s} />)}</div>
                </div>
                <VDivider />
